...
if GOOGLE_API_KEY:
    ...
    st.warning("`GOOGLE_API_KEY` not found in Streamlit Secrets. AI features will be disabled.")
...
def prepare_ai_datasource(data_tuple):
    ...
    This robust version handles potential errors during the merge process.
    """
...
def AI_Overviews(prompt, data_for_ai_tuple, stock_name):
    """
    Handles the interaction with the Google Generative AI model for a single stock.
    This version writes the output directly using st.write().
    """
        ...
        if stock_name:
            if stock_name in processed_data["Name"].values:
                ...
                with st.spinner(f"AI กำลังวิเคราะห์ข้อมูล..."):
                    response = model.generate_content([prompt, csv_string])
                    st.markdown(response.text)

            else:
                st.error(f"ไม่พบหุ้น '{stock_name}' ในข้อมูล")
        else:
            # This case might not be triggered with the old UI but is good for safety
            st.warning("กรุณาระบุชื่อหุ้นที่ต้องการวิเคราะห์")
...
def AI_Market_Summary(prompt, data_for_ai_tuple):
    ...
    try:
        ...
        with st.spinner(f"AI กำลังสร้างบทสรุปภาพรวมตลาด..."):
            response = model.generate_content([prompt, csv_string])
            st.markdown(response.text)
            
    except Exception as e:
        st.error(f"เกิดข้อผิดพลาดระหว่างการสร้างบทสรุป: {e}")
